...
class Person:
    ...
    def __repr__(self):
        ...
        return (
            f"Person(name:{self.name},uuid:{self.uuid},nationality:{self.nationality}, gender:{self.gender},research_interest:{self.research_interest},paper_count:{self.paper_count},papers:{papers})"
        )
...
class Data:
    ...
    # This function returns  unique ID's of researchers with their research Interests.
    ...
    def get_researchInterests(self, G, personIds, researchInt_ids):
        """
        This function returns dictionaries of researchers with their research Interests,
        dictionaries of researchers unique ID's with their research Interests
    ...
    # This function returns the similarity matrix and hop distance matrix w.r.t to every users and research Interests
    #  to compute the Similarity_matrix for all available users in the graph use this instead of p_ids. totalPeopleIds.
    ...
    def get_personsList(self, G, file_path_with_mapped_research_interest):
        """
        This function creates and returns person list with paper count of researcher
        w.r.t. papers he has written.
        """
        ...
        s = df.mask(df == 0).stack().astype(int).astype(str).reset_index(level=1).apply("-".join, 1).add(",").sum(level=0).str[:-1]
        ...
        for node_id in self.personsIds:
            ...
            if ("staffType" in G.nodes[node_id].keys() and G.nodes[node_id]["staffType"] == "Academic") or G.nodes[node_id]["label"] == "External person":
                ...
                nation = G.nodes[node_id]["nationality"] if (G.nodes[node_id]["label"] == "Person") else G.nodes[node_id]["country"]
                ...
                for auth_id, researchOutput_id in search_edges(G, {"==": [("label",), "writtenBy"]}):
                    if auth_id == node_id:
                        ...
                        papers.append(
                            Paper(uuid=researchOutput_id, title=G.nodes[researchOutput_id]["title"], abstract=G.nodes[researchOutput_id]["abstract"], pType=G.nodes[researchOutput_id]["type"])
                        )
                        pc = []
                        ri = []
                ...
                persons.append(Person(uuid=node_id, name=G.nodes[node_id]["name"], nationality=nation, gender=gen, research_interest=ri, paper_count=pc, papers=papers))

        return persons

    def write_scoresToList(self, sim_matrix, hop_matrix):
        """
        This function returns persons list, scores_List for every researcher w.r.t. other researcher.
        """
        persons_dict = {}
        scores_List = []
